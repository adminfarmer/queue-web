...
export class QueueCallerComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (servicePoints.length == 1) {
      this.servicePointId = servicePoints[0].service_point_id;
      this.servicePointName = servicePoints[0].service_point_name;
  ...
  async getListOne() {
    try {
      const rs: any = await this.servicePointService.listOne(this.servicePointId);
      if (rs.statusCode === 200) {
        this.points = rs.results;
        this.servicePointName = this.points[0].service_point_name;
        this.connectWebSocket();
        this.getAllList();
  ...
  async getcreate() {
    try {
      const rs: any = await this.queueService.getcreate(this.servicePointId);
      if (rs.statusCode === 200) {
        this.createItems = rs.results;
    ...
    if (this.hn) {
      if (this.servicePointId) {
        ...
        try {
          ...
          // console.log(error);
          this.alertService.error('HN ผู้รับบริการได้ มีคิว แผนแกนี้แล้ว');
        }

      } else {
        console.log('ไม่ได้เลือกแผนก สถนานบริการ');
        this.alertService.error('ไม่ได้เลือกแผนก สถนานบริการ');
      }
    } else {
      console.log('ไม่พบ HN ผู้รับบริการ');
